"""
Goals page generator for Year Planner.

Generates a single page with a Goals table for long-term goal planning.
"""
...
def generate_goals_page(document: Document, config: Config) -> None:
    ...
    Creates a single page with a Goals table. The table fills the page
    vertically with configurable columns and rows. Starts on recto,
    followed by a blank verso.
    ...
    # Add blank verso page so next section starts on recto
    # Use minimize_height=True to avoid overflow since table fills the page
...
def _create_goals_table(document: Document, config: Config,
    ...
    - Content rows: Empty cells for user to fill in
    ...
    # Goals table has no header row, only title row
    ...
    for row_idx in range(num_rows):
        ...
        for col_idx in range(num_columns):
            ...
            # Leave cells empty for user to fill in
